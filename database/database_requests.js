...
class DatbaseRequests {
    ...
    addUser = (props) => {
        try {
            ...
            const newSoldier = await db.soldiers.create({ soldier_type: soldier_type, soldier_tash_type: soldier_tash_type, money_addons: money_addons, additional_income: additional_income, days_home: days_home, user_id: newUser.user_id, is_lone_soldier: props.is_lone_soldier, money_data_id: moneyData.money_data_id });
            return true;
        } catch (error) {
            console.log(error);
            return false;
